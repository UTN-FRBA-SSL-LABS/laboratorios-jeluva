...
> **R:**
>
> Porque `#include` no es una referencia: el preprocesador copia el contenido
> completo de cada header dentro del archivo. `stdio.h` y `stdlib.h` arrastran
> ademas los headers internos de glibc, con cientos de typedefs, prototipos y
> macros. Las 94 lineas propias quedan al final del `.i`, precedidas por todo
> lo que se incluyo.
...
> **R:**
>
> No devuelve nada. Los comentarios no son parte del lenguaje C sino de la
> notacion del fuente: el preprocesador los reemplaza por un espacio antes de
> que el compilador vea el codigo. Como no aportan significado al programa,
> desaparecen del `.i`.
...
>
> ```
> $ grep -n "CUADRADO" programa.i
> 2028:    printf("CUADRADO(%d)      = %d\n", 5, ((5) * (5)));
>
> La unica aparicion es dentro de la cadena de formato, que es texto literal.
> La invocacion CUADRADO(LIMITE) ya no existe: quedo expandida a ((5) * (5)).
> ```
...
>
> ```
> $ grep -n '"1\.0"' programa.i
> 2019:    printf("=== Laboratorio de Compilacion en C (v%s) ===\n\n", "1.0");
> ```
...
> **R:**
>
> Son marcadores de linea con el formato `# <linea> "<archivo>"`. Le indican al
> compilador de que archivo y de que linea original proviene el bloque que sigue,
> para que los errores y la informacion de depuracion apunten al fuente real y no
> al `.i`. La declaracion de `printf` viene de `/usr/include/stdio.h`.
...
>
> ```
> $ grep "area_circulo" programa.s
> 	.string	"area_circulo(%.1f) = %.4f\n"
> 	call	area_circulo@PLT
>
> Aparece como texto de la cadena de formato y como una instruccion `call`.
> No hay ninguna etiqueta `area_circulo:` que abra su cuerpo.
> ```
...
> **R:**
>
> `sumar:` es la etiqueta que marca el inicio de la funcion. Las directivas
> `.LFB6` y `.cfi_*` son metadatos para el desenrollado de pila, no generan
> codigo. `endbr64` es la marca de destino valido de salto indirecto (CET).
> `pushq %rbp` y `movq %rsp, %rbp` arman el marco de pila: guardan el puntero
> de marco del llamador y lo reapuntan al marco de esta funcion.
...
> 	movl	llamadas(%rip), %eax
>
> Si aparece: como simbolo global reservado en memoria y en los accesos
> relativos a %rip que la leen y la escriben.
> ```
...
> **R:**
>
> Porque cada `.c` es una unidad de traduccion independiente. `programa.c` solo
> vio el prototipo de `area_circulo` en `matematica.h`: sabe como llamarla, pero
> no tiene su cuerpo, asi que la anota como `U` (indefinida) y deja una entrada
> de reubicacion. `matematica.c` si contiene la definicion, por eso alli el
> simbolo sale como `T`, con su codigo en la seccion `.text`.
...
>
> Le faltan las referencias resueltas y el codigo de arranque del proceso
> (`crt1`), que aporta libc durante el enlazado.
> ```
...
> **R:**
>
> Quedan porque libc se enlaza de forma dinamica: el enlazador no copia el codigo
> de `printf` ni de `puts` dentro del ejecutable, solo deja anotado el nombre y la
> version del simbolo. Los resuelve el cargador dinamico (`ld.so`) al lanzar el
> proceso, mapeando `libc.so.6` en memoria y completando la GOT/PLT.
...
> **R:**
>
> `CUADRADO(x)` es una sustitucion de texto: desaparece en el preprocesamiento,
> antes de que el compilador vea el codigo. No tiene tipos ni verificacion, y
> evalua su argumento tantas veces como aparezca en la expansion, por eso
> `CUADRADO(n++)` incrementa `n` dos veces.
>
> `sumar(a, b)` es una funcion real: sobrevive hasta el ensamblado como simbolo
> y desaparece recien en el enlazado, cuando su llamada queda resuelta a una
> direccion. Tiene firma, el compilador verifica cantidad y tipo de argumentos, y
> evalua cada argumento una sola vez. El costo es la llamada (armado de marco de
> pila y retorno), que la macro se ahorra.
...
> **R:**
>
> `T` es un simbolo definido en la seccion `.text`: codigo ejecutable, tipicamente
> una funcion. `D` es un simbolo definido en la seccion `.data`: una variable
> global con valor inicial distinto de cero, que se guarda en el archivo objeto
> porque el programa necesita leerla al arrancar. Una global inicializada en cero
> va a `.bss` y aparece como `B`: alli solo se anota el tamano, el sistema la pone
> en cero al cargar el proceso. En este lab `llamadas = 0` sale como `B`.
...
> ```
> $ ldd programa
> 	linux-vdso.so.1 (0x0000765090130000)
> 	libc.so.6 => /usr/lib/x86_64-linux-gnu/libc.so.6 (0x000076508fe00000)
> 	/lib64/ld-linux-x86-64.so.2 (0x0000765090132000)
> ```
...
> **R:**
>
> Porque `gcc` no es el enlazador sino un driver: invoca a `ld` con una linea de
> comando armada por el, que ya incluye `-lc` y los objetos de arranque
> (`crt1.o`, `crti.o`, `crtn.o`). Todo programa C los necesita, asi que agregarlos
> a mano seria ruido. Si se invocara `ld` directamente habria que pasarlos
> explicitamente.
